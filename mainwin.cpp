...
Mainwin::Mainwin() {
    ...
    //         M A N A G E R
    // (Server, Manager, Owner) “Create a New Manager” Append Manager to the Create menu
  ...
  //         U N D O
    // (ALL) Undo any actions taken previously
    menuitem_undo = Gtk::manage(new Gtk::MenuItem("_Undo...", true));
    //menuitem_undo->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_about_click));
    editmenu->append(*menuitem_undo);

//         R E S T O C K
    // (Server) Server restocks all the items in the shop
    menuitem_restock = Gtk::manage(new Gtk::MenuItem("_Restock...", true));
   // menuitem_restock->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_undo_click));
    editmenu->append(*menuitem_restock);

//         C U S T O M E R
    // (Owner,Server,Manager ) Edit the customer info of the shop
   menuitem_customer = Gtk::manage(new Gtk::MenuItem("_Customer...", true));
    menuitem_customer->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_customer_click));
    editmenu->append(*menuitem_customer);
 
//         S E R V E R
    // (Owner, Manager) Edit the server info of the shop
    menuitem_server = Gtk::manage(new Gtk::MenuItem("_Server...", true));
    menuitem_server->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_server_click));
    editmenu->append(*menuitem_server);

//         M A N A G E R
   //  (Owner) Edit the manager info of the shop
    menuitem_manager = Gtk::manage(new Gtk::MenuItem("_Manager...", true));
    menuitem_manager->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_manager_click));
    editmenu->append(*menuitem_manager);

//         O W N E R
    // (Owner) Edit the Owner info of the shop
    menuitem_owner = Gtk::manage(new Gtk::MenuItem("_Owner...", true));
    menuitem_owner->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_owner_click));
    editmenu->append(*menuitem_owner);
    ...
    menuitem_fill_order = Gtk::manage(new Gtk::MenuItem("Fill Order", true));
    ...
    menuitem_pay_order = Gtk::manage(new Gtk::MenuItem("Pay for Order", true));
    menuitem_pay_order->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_pay_order_click));
    processmenu->append(*menuitem_pay_order);
    ...
    set_title("Mav's Ice Cream Emporium | Powered by Indus Inc.");
}
...
void Mainwin::on_all_click() {
    ...
    menuitem_pay_order->set_sensitive(true);
...
void Mainwin::on_customer_click() {
    ...
    menuitem_pay_order->set_sensitive(false);
    menuitem_cancel_order->set_sensitive(true);
